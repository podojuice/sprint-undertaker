...
- [ ] 앱 UX 개선
  ...
  - [ ] UI/브랜딩 다듬기
    ...
    - 타이포/컬러/레이아웃 정리
    - 전체 제품 톤앤매너 정리
- [x] Claude status line — 제거 결정 (매 턴마다 보이지만 내용이 자주 안 바뀌어 불필요하다 판단)
- [x] Claude project skill / command 추가
  - [x] `/undertaker:character-status` skill — 캐릭터 카드, 스탯, unread notification 표시
  - [x] `/undertaker:titles` skill
  - [x] `/undertaker:help` skill
  - [x] `/undertaker:project` skill — 주간 프로젝트 진행률 바 + 종료일
  - [x] `/undertaker:profile` skill — 공개 프로필 조회
  - [x] `/undertaker:login`, `/undertaker:register` skill
...
- [x] 칭호 조건 하드코딩 제거
  ...
  - DB sync 스크립트 추가
- [x] 실제 Claude Code 환경에서 hook 설치 end-to-end 검증 — 로컬 서버로 정상 동작 확인
- [x] GitHub repo public 전환 + 오픈소스 (MIT 라이선스, README)
- [x] Claude Code 마켓플레이스 배포
  - `.claude-plugin/marketplace.json` + `plugin.json` 루트에 추가
  - `/plugin marketplace add https://github.com/podojuice/sprint-undertaker` 로 설치 가능
- [ ] 프로덕션 배포
  - [ ] AWS App Runner로 배포 (`apprunner.yaml` 추가 완료)
  - [ ] DB: Supabase (이미 연결됨) 또는 RDS
  - [ ] 배포 후 `register/login.py` 기본 서버 URL 업데이트
  - [ ] 공식 Claude Code 마켓플레이스 심사 제출
...
배포는 AWS App Runner 기준으로 진행한다. PostgreSQL은 Supabase 또는 RDS를 별도 운영한다.
